...
library(tidyverse)
data(diamonds)
bigDiamonds <- subset(diamonds, diamonds$carat>1)
```
...
```{r}
summarySE(diamonds, measurevar="price", groupvars= "cut")
```
...
# A second to code using dplyr
diamonds %>% 
  group_by(cut) %>%
  dplyr::summarize(Mean= mean(price), Sd= sd(price),sd(price/sqrt(length(cut))))
```
...
# summary of 
# mtcars2 <- summarySE(mtcars, measurevar= "mgp", groupvars= "cyl")
# mtcars2
```
...
```{r}
ggplot(diamonds, aes(prices))+ geom_histogram()
```
...
```{r}
ggplot(diamonds, aes(prices))+ geom_histogram()+theme_classic()
```

```{r}
ggplot(diamonds, aes(prices))+geom_histogram()+theme_apa()

# only work if you install 'jtools'
```
...
```{r}
ggplot(diamonds, aes(price))+ geom_histogram(color="orange", fill="blue" + themes_classic()

# You can write the name of the color or use the code                                          
