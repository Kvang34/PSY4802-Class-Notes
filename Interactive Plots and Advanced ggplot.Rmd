...
penguinPolarBearEmotion <- penguinData %>% 
  ...
  summarise(meanPenguinsBorn= mean(penguinsBorn))
```
...
# position_dodge() help separate the bar graph, not stack them  
```
...
ggplot(penguinData, aes(emotionsOfPenguins, penguinsBorn, fill=polarbearPresence))+ 
  geom_boxplot()+ facet_wrap(~polarbearPresence)
...
ggplot(penguinData, aes(fishAvailable, penguinsBorn))+ facet_wrap(~polarbearPresence)+geom_point()+geom_smooth(method="lm")
```

using color 
```{r}
ggplot(penguinData, aes(fishAvailable, penguinsBorn, color=polarbearPresence))+geom_point()+geom_smooth(method="lm")
```
...
# shiny is website that have interactive graphs made with R
```
...
`sliderInput(inputId, label, value, min, max)`
...
# label is the label people will see
# value is the location of the dot on a scale of 0-100 when you run the code 
...
# it will give us 32 random number 
```
...
ui <- fluidPage(
  sliderInput(inputId = "number", 
    label = "Pick a number", 
    value = 13, min = 0, max = 100),
  plotOutput("hist") # tell shiny to make a histogram 
)

# make histogram based on info in server 
server <- function(input, output) {
...
} #to make the boxplot, we have to change geom_histogram() to geom_boxplot()
...
ui <- fluidPage(
  sliderInput(inputId = "number1", 
    label = "Pick a number", 
    value = 2500, min = 1, max = 5000),
  plotOutput("scatterplot"), sliderInput(inputId = "number2", 
    label = "Pick a number", 
    value = 2500, min = 1, max = 5000),
...
server <- function(input, output) {
  output$scatterplot <- renderPlot({
    ggplot(, aes(rnorm(input$number1, nrom(input$number2))))+ geom_point(color="purple")+theme_classic()
  })
}
