...
```{r}
#last week we learned to download the data and get the values of one column by doing 'dataName$columnName'. For example, 
...
We'll use diamonds to learn about: `colnames(dataname)`, `names()`, and 0 row
```{r}
#in order to use the diamind, we need to download the ggplot library

library(ggplot2)

#You know the package is install by checking the package tab on the lower right corner 

data(diamonds)
colnames(diamonds)

#you could also use the name function 'name(dataName)' and this would give you the same code. Or using the bracket 'dataName[row,column]'

#For example, if we want just row 0 
diamonds[0,]
...
```{r}
#If we do not remeber the column name of the data, we could do 'colnames(dataName)[columnNum]'

colnames(diamonds)[3]

#When we run the code, it tell us that the thrid column is 'color'. We check the data and it is right 
```
...
```{r}
#You could change the name of the column by doing 'colnames(dataame)[columnNum] <- 'newColumnNam''

#For example, we want to change color to COLOR
...
#So if we want to change the name into upper or lower case, we could use the function 'toupper()' or 'tolower()'

toupper("why are you yeLLing")

#If we want to change all of the column name lowercase, we do 
colnames(ChickWeight) <- tolower(colnames(ChickWeight))
colnames(ChickWeight)

#When we run the code, all of the column will be in lower case 
...
```{r}
#We could add a new column by 'dataName$newCol <- newRow' 

diamonds$size <- NA

#When we run the code, we got a new column called size with NA in each rows 
```
Then you can fill it with whatever information you want. 
```{r}
diamonds$size <- "big"

#to make the data alter from "big small big"
diamonds$size <- rep(c("big", "small"),nrow(diamonds)/2)
```
or you can fill the new column in based on other data in the dataframe
```{r}
#dataName$newColumnNam <- dataName$columnName
...
#by reflect, you enter "?mtcars" in the console and search what the variables means. When you enter the code, it tells you that "am represent transmission (0=automatic, 1=manual)"
```
...
```{r}
#If we don't know what type of class the values are, we could check them in R using the code 'class()'. For example, if we want to know the class oc carat in the diamond data we do: 

class(diamonds$carat)

#when we run the code, it tells us that the class for this column is numeric 
```
...
#when we run the code, it tell us that it is true that class for 'transmission' is a factor 
...
#when we run the code, we notice the variables for 'mtcars$hp' give us a the mean, max, min, sq. 

#For 'mtcars$hp2', we got a category for how many cars got that horsepower  
```
##IMPORTANT TO DOUBLE CHECK DATA
```{r}
mtcars$hp2
mtcars$hp3 <- as.numeric(mtcars$hp2)
mtcars$hp3

#If we want to convert hp2 into a numeric number, we can't do 'numberic(mtcars$hp2)". Instead, we would do the folliwng: 'as.newClass(levels(dataName$columnName)[dataName$columnName])'
...
#When we run the code, we see we've created a vector called plusTwo 

#If we want to put it into a data, we do 
...
#If we want to put into a data set 
sevenDat <- data.frame(numbers)
sevenDat$timesSeven <- multiplySeven(numbers)

#If we want to save test data frame
...
#'paste()' let you paste two diffeent things together. It also help you remove the quotation between the phrases. FOr example, when we did 

favoriteThings <- function(things){
  c("my favorite is", things)
}

#This code give us "my favorite is" "green tea"

#If we use, 'paste()', it give us "my favorite is green tea"

favoriteThings <- function(things){
  paste("my favorite is", things)
}
```
test
```{r}
...
```{r}
#create a vector for numbers counting backward
backwards <- 10:1
backwards
```
```{r}
...
```{r}
columnMean <- function(column1, column2){
...
```{r}
columnMean(mtcars$mpg,mtcars$hp)
...
to make a for loop: 
for (i in 1: (individual in bigger group)){
...
#i is a placeholder for anything that we want to put into the loop (input)

#If we do "1 in 200:nrow()...", this would run the loop/code starting at 200 
```
...
#You could make a vector 
testInput <- 1:100
for (i in 1:length(testInput)){
  print(i*4)
}
#You can also make a data frame 

#testOutput <- rep(NA, length(testInput))
#test <- cbind.data.frame(testInput, testOuput)
#for(i in 1:nrow(test)){
  #test$testOutput[i] <- test$testInput[i] * 4}
```
...
#create a for loop
for (i in 1: nrow(mtcars)){ #you can use 'length(mtcars$mpg)'
  mtcars$mileage[i] <- mtcars$mpg[i]> 25 
}
```
...
```{r}
#create an if statement for the first car
...
for (i in 1: nrow(mtcars)){ 
  mtcars$goodMileage2[i] <- if(mtcars$mpg[i]>25){
    mtcars$goodMileage2[i] <- "good mileage"
  } else {mtcars$goodMileage2[i] <- "good mileage"}
  }
```
...
} else { print("less than 100")}
}

#another way is... 
ChickWeight$weight2 <- ifelse(ChickWeight$weight>100, "greater than 100", "less than 100")
...
#looking at the help, it shows that 0= automatic, 1=manual 


#if we have more than one if else statement 
